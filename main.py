...
import utils.privacy_tests.membership_inference_attack.membership_inference_attack as mia
from utils.membership_inference_attack.data_structures import AttackInputData
from utils.membership_inference_attack.data_structures import SlicingSpec
from utils.membership_inference_attack.data_structures import AttackType
...
def cosine_similarity(weights1, weights2):
    # print(1)
    # norm_1 = np.linalg.norm(weights1 + 1e-9)
    # np.sum(np.abs(weights1+1e-9)**2,axis=-1)**(1./2)
    # print(2)
    # norm_2 = np.linalg.norm(weights2 + 1e-9)
    # np.sum(np.abs(weights2+1e-9)**2,axis=-1)**(1./2)
    # print(3)
    # mul_res = np.dot(weights1, weights2)
    # print(4)
    # return mul_res/(norm_1 * norm_2)
...
def train_fn(X_train_shards, Y_train_shards, X_test, Y_test, return_dict,
    ...
    while return_dict['eval_success'] < gv.max_acc and t < args.T:
        ...
        if args.mal and args.mia:
            os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
            os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
            print('Start membership inference attack.')
            import tensorflow.python.keras.backend as K
            if 'MNIST' in args.dataset:
                K.set_learning_phase(0)
            target_model = model_mnist(type=args.model_num)
            ...
            sample_weight = np.load(gv.dir_name + 'ben_weights_%s_t%s.npy' % (0, t), allow_pickle=True)
            ...
            target_model.set_weights(sample_weight)
            X_miadata = X_train_shards[0]
            Y_miadata = Y_train_shards[0]
            X_miatest = X_test
            Y_miatest = np_utils.to_categorical(Y_test, gv.NUM_CLASSES)

            # X_miadata = X_train_shards[0]
            # Y_miadata = Y_train_shards[0]
            # X_miatest = X_train_shards[1]
            # Y_miatest = Y_train_shards[1]


            # X_miadata = X_test
            # Y_miadata = np_utils.to_categorical(Y_test, gv.NUM_CLASSES)
            # X_miatest = X_train_shards[0]
            # Y_miatest = Y_train_shards[0]

            # X_miadata = X_train_shards[0][0:len(X_train_shards[0])//2]
            # Y_miadata = Y_train_shards[0][0:len(Y_train_shards[0])//2]
            # X_miatest = X_train_shards[0][len(X_train_shards[0])//2:len(X_train_shards[0])-1]
            # Y_miatest = Y_train_shards[0][len(Y_train_shards[0])//2:len(Y_train_shards[0])-1]

            prob_train = np.zeros((len(X_miadata), gv.NUM_CLASSES))
            prob_test = np.zeros((len(X_miatest), gv.NUM_CLASSES))
            loss_train = np.zeros((len(X_miadata)))
            loss_test = np.zeros((len(X_miatest)))
            for i in range(int(len(X_miadata) / gv.BATCH_SIZE)):
                X_data_slice = X_miadata[i * (gv.BATCH_SIZE):(i + 1) * (gv.BATCH_SIZE)]
                Y_data_slice = Y_miadata[i * (gv.BATCH_SIZE):(i + 1) * (gv.BATCH_SIZE)]
                Y_data_slice = np.argmax(Y_data_slice, axis=-1)
                # print(Y_test_slice)
                pred_np_i = sess.run(logits, feed_dict={x: X_data_slice})
                loss_i = sess.run(loss, feed_dict={x: X_data_slice, y: Y_data_slice})
                # pred_np_i = sess.run(tf.nn.softmax(pred_np_i))
                prob_train[i * gv.BATCH_SIZE:(i + 1) * gv.BATCH_SIZE, :] = pred_np_i
                loss_train[i * gv.BATCH_SIZE:(i + 1) * gv.BATCH_SIZE] = loss_i
            for j in range(int(len(X_miatest) / gv.BATCH_SIZE)):
                X_test_slice = X_miatest[j * (gv.BATCH_SIZE):(j + 1) * (gv.BATCH_SIZE)]
                Y_test_slice = Y_miatest[j * (gv.BATCH_SIZE):(j + 1) * (gv.BATCH_SIZE)]
                Y_test_slice = np.argmax(Y_test_slice, axis=-1)
                # print(Y_test_slice)
                pred_np_j = sess.run(logits, feed_dict={x: X_test_slice})
                # pred_np_j = sess.run(tf.nn.softmax(pred_np_j))
                loss_j = sess.run(loss, feed_dict={x: X_test_slice, y: Y_test_slice})
                prob_test[j * gv.BATCH_SIZE:(j + 1) * gv.BATCH_SIZE, :] = pred_np_j
                loss_test[j * gv.BATCH_SIZE:(j + 1) * gv.BATCH_SIZE] = loss_j
            # cce = tf.keras.backend.categorical_crossentropy
            # constant = tf.keras.backend.constant    
            # loss_train = cce(constant(Y_miadata), constant(prob_train), from_logits=True)
            # loss_test = cce(constant(Y_miatest), constant(prob_test), from_logits=True)
            # loss_train = loss_train.eval(session=sess)
            # loss_test = loss_test.eval(session=sess)
            print("prob_train:", prob_train[0])
            print("prob_test:", prob_test[0])
            print("loss_train:", loss_train[0:10])
            print("loss_test:", loss_test[0:10])
            ...
            attack_input = AttackInputData(
                            ...
                            loss_test = loss_test,
                            labels_train = np.argmax(Y_miadata, axis=1),
                            labels_test = np.argmax(Y_miatest, axis=1)
                            # labels_train = Y_miadata,
                            # labels_test = Y_miatest
                            )
            ...
            import utils.membership_inference_attack.plotting as plotting
            print(plotting.plot_roc_curve(attacks_result.get_result_with_max_auc().roc_curve))
            with open('output/mia_auc.txt', 'a') as f:
                f.write(str(attacks_result.get_result_with_max_auc().roc_curve.get_auc())+'\n')
            with open('output/mia_advantage.txt', 'a') as f:
                f.write(str(attacks_result.get_result_with_max_auc().roc_curve.get_attacker_advantage())+'\n')
...
if __name__ == "__main__":
    ...
    if args.mia:
        with open('output/mia_auc.txt', 'w') as f:
            f.write('MIA AUC\n\n')
        with open('output/mia_advantage.txt', 'w') as f:
            f.write('MIA Advantage\n\n')
